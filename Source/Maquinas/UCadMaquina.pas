...
procedure TfrmCadMaquinaVeiculo.btnBuscarListaClick(Sender: TObject);
begin
 Filtro;
end;
...
begin
 ...
 if edtHorimetroAtual.Text.Length>0 then
  dbCtx.TMaquinashorimetro.AsString := edtHorimetroAtual.Text;

 dbCtx.TMaquinashorimetroxkm.AsInteger       := cbxtipoMedicao.ItemIndex;
 ...
 try
  dbCtx.TMaquinassyncfaz.AsInteger := 0;
...
begin
 ...
 cbxtipoMedicao.ItemIndex     := dbCtx.TMaquinashorimetroxkm.AsInteger;
 edtHorimetroAtual.Text       := dbCtx.TMaquinashorimetro.AsString;
...
procedure TfrmCadMaquinaVeiculo.btnVizualizarClick(Sender: TObject);
var
 Stream : TMemoryStream;
begin
 cbxCategoria.ItemIndex       := dbCtx.TMaquinasidcategoria.AsInteger;
...
begin
 ...
 if edtNomeFiltro.Text.Length>0 then
   vFiltro := ' AND MODELO LIKE '+QuotedStr('%'+edtNomeFiltro.Text+'%');
...
procedure TfrmCadMaquinaVeiculo.cbxCategoriaFKeyDown(Sender: TObject;
